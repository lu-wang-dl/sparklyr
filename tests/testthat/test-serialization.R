...
test_that("NA values survive Spark roundtrips", {
  skip("failure: NA values survive Spark roundtrips")
  n <- 10
  df <- data.frame(
    int = as.integer(1:n),
    dbl = as.double(1:n),
...
test_that("collect() can retrieve all data types correctly", {
  ...
  library(dplyr)
  skip("Error: The to_utc_timestamp function has been disabled")
...
test_that("collect() can retrieve logical columns with NAs", {
  skip("failure: `logical_nas` not equal to logical_nas_tbl %>% dplyr::collect().")
  expect_equal(
    logical_nas,
